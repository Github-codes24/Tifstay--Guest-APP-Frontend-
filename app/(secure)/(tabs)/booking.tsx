...
const fetchHostelOrders = async (tab: "pending" | "confirmed" | "rejected"): Promise<Order[]> => {
  ...
  if (tab === "pending") {
    hostelUrl = `${BASE_URL}/api/guest/hostelServices/getPendingHostelBookings`;
  } else if (tab === "confirmed") {
    hostelUrl = `${BASE_URL}/api/guest/hostelServices/getConfirmedHostelBookings`;
  } else {
    hostelUrl = `${BASE_URL}/api/guest/hostelServices/getRejectedHostelBookings`;
  }
...
const fetchTiffinOrders = async (tab: "pending" | "confirmed" | "rejected"): Promise<Order[]> => {
  ...
  if (tab === "pending") {
    tiffinUrl = `${BASE_URL}/api/guest/tiffinServices/getPendingTiffinOrder`;
  } else if (tab === "confirmed") {
    tiffinUrl = `${BASE_URL}/api/guest/tiffinServices/getConfirmedTiffinOrder`;
  } else {
    tiffinUrl = `${BASE_URL}/api/guest/tiffinServices/getRejectedTiffinOrder`;
  }
...
const updateDepositStatus = async (bookingId: string, status: "accepted" | "rejected") => {
  ...
  const url = `${BASE_URL}/api/guest/hostelServices/updateGiveDeposit/${bookingId}`;
  ...
  const handleSeeDetails = (order: Order) => {
    ...
    if (order.serviceType === "hostel") {
      ...
      params = {
        ...
        type: order.serviceType,
        bookingId: order.bookingId,
        tiffinServiceName: order.serviceName,
        customer: order.guestName || "You",
