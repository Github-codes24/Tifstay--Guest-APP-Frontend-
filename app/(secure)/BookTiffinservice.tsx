...
import { calender, location1, person, profile, wallet } from '@/assets/images';
...
export default function BookTiffinScreen() {
  ...
  const RadioButton = ({
    ...
    <TouchableOpacity style={styles.radioRow} onPress={() => onPress(value)}>
      <View style={styles.radioOuter}>{selected === value && <View style={styles.radioInner} />}</View>
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <TextInput
          style={[styles.input, { height: 80 }, orderType === 'dining' && { backgroundColor: '#eee' }]}
      ...
      <View style={styles.section}>
        ...
        <TextInput style={styles.input} value={numberOfTiffin} keyboardType="numeric" onChangeText={setNumberOfTiffin} />
        ...
        {Object.entries(sameAsSelections).map(([key, value]) => (
          <View style={styles.checkboxRow} key={key}>
            <Checkbox checked={value} onPress={() => toggleSameAs(key as keyof typeof sameAsSelections)} />
            <Text style={styles.checkboxLabel}>{key === 'sameForAll' ? 'Same For All' : `Same As ${key.slice(-1)}`}</Text>
          </View>
        ))}
        ...
        {(['breakfast', 'lunch', 'dinner'] as MealType[]).map((meal) => (
          <View style={styles.checkboxRow} key={meal}>
            <Checkbox checked={mealPreferences[meal]} onPress={() => toggleMealPreference(meal)} />
        ...
        <RadioButton label="Veg" value="veg" selected={selectedfood} onPress={setSelectedfood} />
        <RadioButton label="Non-Veg" value="nonveg" selected={selectedfood} onPress={setSelectedfood} />
        <RadioButton label="Both" value="both" selected={selectedfood} onPress={setSelectedfood} />
        ...
        {['dining', 'delivery'].map((type) => (
          <View style={styles.checkboxRow} key={type}>
            <TouchableOpacity style={[styles.checkboxBase, orderType === type && styles.checkboxSelected]} onPress={() => setOrderType(type as any)}>
      ...
      <Text style={styles.confirmationText}>Provider will reach out within 1 hour to confirm.</Text>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  section: { borderWidth: 1, borderColor: '#ccc', borderRadius: 10, padding: 15, marginBottom: 25 },
  sectionTitle: { fontWeight: 'bold', fontSize: 16, marginBottom: 10, },
  label: { fontSize: 14, marginBottom: 5, color: '#444',fontWeight:'bold' },
  input: { borderWidth: 1, borderColor: '#aaa', borderRadius: 6, paddingHorizontal: 10, paddingVertical: 8, marginBottom: 15, fontSize: 14 },
  pickerWrapper: { borderWidth: 1, borderColor: '#aaa', borderRadius: 6, marginBottom: 15 },
 pickerInput: {
  height: 51,              
  paddingVertical: 4,      
  ...
  checkboxBase: { width: 24, height: 24, borderRadius: 4, borderWidth: 2, borderColor: '#ccc', backgroundColor: 'white', justifyContent: 'center', alignItems: 'center', marginRight: 8 },
  ...
  radioOuter: { height: 20, width: 20, borderRadius: 10, borderWidth: 2, borderColor: '#004AAD', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
  radioInner: { height: 10, width: 10, borderRadius: 5, backgroundColor: '#004AAD' },
  radioLabel: { fontSize: 14, color: '#333' },
  datePickerButton: { borderWidth: 1, borderColor: '#aaa', borderRadius: 6, paddingVertical: 12, paddingHorizontal: 10, justifyContent: 'center', marginBottom: 15 },
  datePickerText: { fontSize: 14, color: 'black' },
  summaryBox: { borderWidth: 1, borderColor: '#ccc', borderRadius: 10, padding: 10, marginBottom: 20, backgroundColor: '#fff' },
  summaryTitle: { fontSize: 16, fontWeight: 'bold', marginBottom: 5 },
  summaryPrice: { fontSize: 14, color: '#004AAD', marginBottom: 15 },
  submitButton: { backgroundColor: '#004AAD', borderRadius: 8, paddingVertical: 12, paddingHorizontal: 30, marginBottom: 10 },
